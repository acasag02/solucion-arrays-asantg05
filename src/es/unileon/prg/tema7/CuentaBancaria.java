...
public class CuentaBancaria{
     ...
     */
    int obtenerCredito (){
        return obtenerTitularPrincipal().obtenerCredito();
    }
    ...
    public void sacar (int cantidad){
		if ( !obtenerTitularPrincipal().esMayorEdad() ){
			System.out.println("Los menores de edad no pueden sacar dinero.");
		} else {
			if ( saldo + obtenerCredito() < cantidad ){
				System.out.println("Operacion cancelada: credito insuficiente.");
			} else {
				System.out.println("Vamos a sacar " + cantidad + " ?...");
				saldo = saldo - cantidad;
				System.out.println("...ya hemos sacado " + cantidad +
				   " ? " + saldo);
		    }
		}
    }
